...
$.stockTicker = function(elem,options) { //options currently unused

	var stockList = elem.data('array').split(' '), //creates JS array
	...
	for (i = 0; i < stockLength; i++) { //retrieves JSON data and creates elements
		$.getJSON('https://finance.google.com/finance/info?client=ig&q=' + stockList[i] + '&callback=?', function(response) {
		    ...
		    elem.children('.stockTick').append(stockString); //write to div.stockTick
